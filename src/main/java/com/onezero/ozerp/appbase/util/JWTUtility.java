...
public class JWTUtility implements Serializable {
    ...
    public static final String TENANT_API_KEY_PREFIX = "OZERP";
    public static final String TENANT_ID_LIST = "tenantIdList";
    public static final String IS_CONTEXT_USER = "isContextUser";
	...
	public String generateToken(UserDetails userDetails, List<String> roleLits, boolean isUserEnabled,
			int tenantApiKey, List<Long> tenatIdList, boolean isContextUser) {

		Map<String, Object> claims = new HashMap<>();
		return doGenerateToken(claims, userDetails, roleLits, isUserEnabled, tenantApiKey, tenatIdList, isContextUser);
	}

	private String doGenerateToken(Map<String, Object> claims, UserDetails userDetails, List<String> roleLits,
			boolean isUserEnabled, int tenantApiKey, List<Long> tenatIdList, boolean isContextUser) {

		String token = Jwts.builder().setClaims(claims).setSubject(userDetails.getUsername())
				.setIssuedAt(new Date(System.currentTimeMillis())).claim(ROLES, roleLits)
				.claim(PERMISSIONS, userDetails.getAuthorities()).claim(USER_STATUS, isUserEnabled)
				.claim(TENANT_API_KEY, TENANT_API_KEY_PREFIX + tenantApiKey)
				.claim(TENANT_ID_LIST, tenatIdList)
				.claim(IS_CONTEXT_USER, isContextUser)
